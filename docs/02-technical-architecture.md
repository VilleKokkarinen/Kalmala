...
This query adds no world field, terrain deformation, island placement, collision volume, RPC, replicated value, or saved data. It does not prove connection to an open ocean, query inland lake depth, or guarantee that a streaming patch is active. Future gameplay callers must supply server-owned identity and positions and check active collision; client queries currently serve only local presentation. Swimming, boats, islands, and streaming changes remain unfinished Phase 7 work.
