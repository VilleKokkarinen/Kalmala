...
Revision 3 routes the existing classifier and terrain-height sampler through `FKalmalaRegionalGeneration`. Field samples carry only world seed/revision/position metadata alongside the four source values; all region weights and shaping are recomputed. `FKalmalaRegionalTuning` owns the semantic scales. Only derived hydrology splines are cached, spatially indexed and identity-keyed, without serialization or replication. Revision-3 water rendering clips variable inland levels against final terrain triangles; the minimap interpolates their depth on that same lattice. `FKalmalaLakeBasin` bypasses the legacy component cache for new analytic enclosed bowls. Existing revision-1/2 queries retain their old behavior. See the Phase 7 contract for source scales, channel construction, and known hydrology limits.

Biome rule selection is versioned through `FKalmalaWorldFieldSample::GeneratorRevision` metadata from the immutable server world identity. Revision 2 adds terrain and moisture suitability to the four-field classifier; revision 1 remains unchanged. New-world entry points now select revision 3 while serialized config defaults remain revision 1. Population save slots for revision 2 and later include seed and revision, preventing new worlds from overwriting the legacy slot. Gameplay still classifies server-owned samples; local rendering consumes the same replicated identity. See `docs/08-world-generation-and-biomes.md` for threshold precedence and limitations.
...
This query adds no world field, terrain deformation, island placement, collision volume, RPC, replicated value, or saved data. It does not prove connection to an open ocean, query inland lake depth, or guarantee that a streaming patch is active. The first gameplay consumer is `UKalmalaCharacterMovementComponent`: server and predicting owner sample the replicated immutable identity at the pawn's current position, enter its generated-ocean custom mode at 100 cm sea depth, and return to normal walking below 75 cm. The server remains authoritative through normal Character Movement replication and correction; the client supplies neither a depth nor a requested mode. The custom mode floats the existing pawn capsule at the zero-height visible sea and retains swept collision with the shared terrain mesh. Swimming, boats, islands, and streaming changes otherwise remain unfinished Phase 7 work.
