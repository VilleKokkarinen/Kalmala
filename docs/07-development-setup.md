...
Shift-click the expanded map to begin a transient personal pin: type a 1–32-character label, choose `1` Cairn, `2` Lantern, or `3` Thread, and press Enter to place. Click a visible pin to mark it complete, Ctrl-click to hide it, and right-click to remove it. These pins are not yet persisted and never create an RPC, discovery claim, route, or gameplay instruction.
