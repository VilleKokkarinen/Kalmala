...
After an editor build, run `Kalmala.Gameplay.InteractionGrid.SurfaceMoisture` headlessly with the standard temporary `-UserDir`, `-abslog`, `-DDC-ForceMemoryCache`, and `-TestExit="Automation Test Queue Empty"` flags. It verifies the 200 cm floor-key transform, deterministic cell centre, bounded rain/drying moisture, malformed-input recovery, and the explicit 1,024-cell server budget. This focused test does not claim grid replication, persistence, player Wet, fire spread, or construction response.
