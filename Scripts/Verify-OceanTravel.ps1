...
try {
    ...
    do {
        ...
        if (($serverText + $clientText) -match 'Fatal error:|Assertion failed:|Ocean travel test could not (resolve|find)') { throw 'Ocean-travel verification failed; inspect logs.' }
        $serverPass = $serverText -match 'Ocean travel test owner entered open ocean. Authority=1.' -and $serverText -match 'Ocean travel test owner reached the seeded island. Authority=1.'
        $clientPass = $clientText -match 'Ocean travel test owner entered open ocean. Authority=0.' -and $clientText -match 'Ocean travel test owner reached the seeded island. Authority=0.'
    ...
    Write-Output 'PASS: host and client crossed the shared generated ocean and reached the same seeded island.'
}
finally {
