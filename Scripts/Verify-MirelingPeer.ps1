...
try {
    ...
    do {
        if ($server.HasExited -or $client.HasExited) { throw 'A Mireling peer exited before verification.' }
        ...
        $ready = $serverText -match 'Mireling verification server: Passed=1 SeedReproduced=1 BoundedActivation=1 InvalidRejected=1 ActionSerial=4 Health=0\.0 PlayerHealth=[0-9]+\.0 Defeated=1 Saved=1 Ash=1 RemoteAsh=0'
        $ready = $ready -and $clientText -match 'Mireling verification client rejected invalid owned attack without target data\.'
