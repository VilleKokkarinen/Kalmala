...
try {
    ...
    Write-Output 'PASS: two client visits to the same live server; each server pawn starts empty before its fixture grant; owner agreement and privacy verified on both visits; server grants/removals and rejection checks, owner-only contents, rejected local client mutations, remote privacy after replication, and read-only local presentation on both peers.'
}
finally {
