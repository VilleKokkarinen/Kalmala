...
try {
    ...
    if (($serverText -match 'World map tile presentation: Seed=418 Tiles=\d+ Fingerprint=\d+ PollOnly=1.') -and
            ($serverText -match 'World map fog presentation: Seed=418 Cells=\d+ Loaded=0 Remote=0.') -and
            ($clientText -match 'Client received world-generation identity: Seed=418') -and
            ($clientText -match 'World map tile presentation: Seed=418 Tiles=\d+ Fingerprint=\d+ PollOnly=1.') -and
            ($clientText -match 'World map fog presentation: Seed=418 Cells=\d+ Loaded=0 Remote=0.')) { break }
        Start-Sleep -Milliseconds 500
    } while ((Get-Date) -lt $deadline)
    if ((Get-Date) -ge $deadline) { throw 'Host/client world-map tile presentation timed out.' }
    $serverMatch = [regex]::Match($serverText, 'World map tile presentation: Seed=418 Tiles=(\d+) Fingerprint=(\d+) PollOnly=1.')
    $clientMatch = [regex]::Match($clientText, 'World map tile presentation: Seed=418 Tiles=(\d+) Fingerprint=(\d+) PollOnly=1.')
    ...
    $server = Start-Process $Editor -WindowStyle Hidden -PassThru -ArgumentList "`"$project`" /Game/Kalmala/Maps/Prototype/L_Prototype?listen -port=$Port -WorldSeed=418 $common -abslog=`"$restartServerLog`" -UserDir=`"$output\Host`""
    $deadline = (Get-Date).AddSeconds(60)
    do {
    ...
    $client = Start-Process $Editor -WindowStyle Hidden -PassThru -ArgumentList "`"$project`" 127.0.0.1:$Port -WorldSeed=999 $common -abslog=`"$restartClientLog`" -UserDir=`"$output\Client`""
    $deadline = (Get-Date).AddSeconds(60)
    do {
        ...
        if (($restartServerText -match 'World map fog presentation: Seed=418 Cells=\d+ Loaded=1 Remote=0.') -and
            ($restartClientText -match 'Client received world-generation identity: Seed=418') -and
            ($restartClientText -match 'World map fog presentation: Seed=418 Cells=\d+ Loaded=1 Remote=0.')) { break }
        Start-Sleep -Milliseconds 500
    } while ((Get-Date) -lt $deadline)
