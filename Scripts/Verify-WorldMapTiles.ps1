...
try {
    ...
    do {
        ...
        if (($serverText -match 'World map tile presentation: Seed=418 Revision=4 Tiles=\d+ Fingerprint=\d+ PollOnly=1.') -and
            ...
            ($clientText -match 'World map tile presentation: Seed=418 Revision=4 Tiles=\d+ Fingerprint=\d+ PollOnly=1.')) { break }
        Start-Sleep -Milliseconds 500
    } while ((Get-Date) -lt $deadline)
    ...
    Write-Output "PASS: $($serverMatch.Value) matches the conflicting-seed client after authoritative identity replication."
}
finally {
