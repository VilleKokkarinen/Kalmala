...
void AKalmalaCharacter::VerifyConstructionMovement(const float DeltaSeconds)
{
    ...
    if (!FParse::Param(FCommandLine::Get(), TEXT("KalmalaConstructionMovementTest"))
        || bConstructionMovementFinished || !GetPlayerState() || !GetWorld()->GetGameState()
    ...
    if (HasAuthority() && !bConstructionMovementSpawned)
    {
        ...
        for (const FName Kit : { FName(TEXT("FloorKit")), FName(TEXT("WallKit")) })
        {
            const FVector Offset = Kit == TEXT("FloorKit") ? FVector::ZeroVector : FVector(0, 90, 122);
    ...
    if (!Floor || !Wall || FMath::Abs(GetActorLocation().Z - Floor->GetActorLocation().Z) > 200) return;
    ...
    if (IsLocallyControlled()) AddMovementInput(FVector::YAxisVector, 1.0f, true);
